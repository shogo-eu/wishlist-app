...
### 6-1. 作成
- ログインユーザーはコレクションを作成できる
- コレクション名はユーザーが自由に設定できる

### 6-2. 編集
- コレクション名の変更が可能
- 表示順（sort）の変更が可能（v1では必須でなくても可）
...
| 処理 | 内容 |
|---|---|
| コレクション削除 | collections から削除 |
| 紐づくアイテム | 未分類（Uncategorized）へ移動 |

---

### 6-4. 分類（アイテム紐づけ）
- 1つのお気に入りアイテムは **複数コレクションに所属可能**
- 分類操作は一覧ページから行う
- PDPでは v1 では分類UIを必須としない

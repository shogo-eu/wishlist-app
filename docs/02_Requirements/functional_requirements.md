...
本ドキュメントでは、Wishlist App（variant + collection）の  
**v1 における機能要件**を定義する。
...
## 6. お気に入りコレクション（ログイン限定）

### 6-1. 目的
ストア側で用意したコレクション（Shopify Collection）を、ユーザーがお気に入り登録できるようにする。
お気に入り登録されたコレクションは、Wishlistページ上で「コレクション一覧」として表示する。

### 6-2. 対象
- 対象は **Shopify Collection** とする
- 保存単位：`collection_id`（数値ID）または `collection_handle`（文字列）
  - v1は **collection_id（数値）推奨**（不変で安全）

### 6-3. 利用条件（ログイン限定）
- コレクションのお気に入り登録/解除は **ログインユーザーのみ**利用可能
- ゲストはコレクションお気に入りのUIを非表示、または押下時にログイン誘導を表示する

### 6-4. 操作
- コレクション一覧セクション（バナー）に♡ボタンを表示する
- クリックでトグル（登録/解除）する

### 6-5. 保存
- 保存先は Customer metafield `wishlist.data` とする
- `wishlist.data` に「お気に入りコレクション一覧」を保持する（詳細はデータ設計に従う）

### 6-6. 表示（Wishlistページ）
- Wishlistページに「お気に入りコレクション一覧」ブロックを表示する
- 登録済みコレクションのみを表示し、クリックで該当コレクションへ遷移できる

### 6-7. 非対象（v1）
- コレクションのユーザー作成（フォルダ作成）
- コレクション名の編集（ストア側定義のため）
- コレクション内へのアイテム分類（アイテム⇔フォルダ紐づけ）
---
...
## 11. 受け入れ条件（Acceptance Criteria / v1）
